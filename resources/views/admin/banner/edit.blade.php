	        ...
	        <div class="portlet light portlet-fit portlet-form bordered" id="form_wizard_1">
	             <div class="portlet-body">
                  ...
                  <form method="POST" action="{{ route('banner.update',$banner->id) }}" class="form-horizontal" enctype="multipart/form-data">
                     <div class="form-body">
                        <h3 class="form-section">{{ __('Banner Edit') }}</h3>
                        @csrf
                        {{ method_field('PUT') }}
                         <div class="form-group  @error('title') has-error @enderror">
                            <label class="control-label col-md-2" for="inputError">{{ __('Title') }}</label>
                                <div class="col-md-8">
                                    <input type="text" class="form-control " name="title" value="{{ old('title',$banner->title) }}"  autocomplete="title" autofocus>
                                     @error('title')
                          ...
                          <div class="form-group  @error('sequence') has-error @enderror">
                              <label class="control-label col-md-2" for="inputError">{{ __('Sequence') }}</label>
                                  <div class="col-md-8">
                                       <input type="text" class="form-control " name="sequence" value="{{ old('sequence',$banner->sequence) }}"  autocomplete="title" autofocus>
                                          @error('sequence')
                                              <span class="invalid-feedback" role="alert">
                                                  <strong>{{ $message }}</strong>
                                              </span>
                          ...
                          <div class="form-group  @error('status') has-error @enderror">
                              <label class="control-label col-md-2" for="inputError">{{ __('Status') }}</label>
                                  <div class="col-md-8">
                                      <input type="radio"  name="status" value="1" @if($banner->status==1) checked="checked" @endif>Yes
                                      <input type="radio"  name="status" value="0" @if($banner->status==0) checked="checked" @endif>No
