...
@section('content')
	<h1 class="page-title"> Service Management
	    <small></small>
	</h1>
	<div class="row">
	    <div class="col-md-12">
			            	...
			            	@if (count($service) >0)
				            	@foreach ($service as $n)
								    <tr>
					                    ...
					                    <td> 
					                    	<a href="{{ route('service.edit',$n->id) }}"  class="btn dark btn-sm btn-outline sbold uppercase"><i class="fa fa-share"></i>
								                Edit
								            </a>&nbsp;
					                    	<a class="btn dark btn-sm btn-outline sbold uppercase" href="{{ route('service.destroy',$n->id) }}"
								            ...
								            <form id="logout-form-{{$n->id}}" action="{{ route('service.destroy',$n->id) }}" method="POST" style="display: none;">
								                {{ csrf_field() }}
								                {{ method_field('DELETE') }}
								            </form>
					                     </td>
